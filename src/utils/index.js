...
export const normalizeInventories = (prods) => {
  if (prods.length) {
    ...
    return products?.map((i) => ({
      label: _.compact([i.product, i.variant.name]).join(" / "),
      ...i,
    }));
  }
  return [];
...
export const normalizeProducts = (products) => {
  return products.length
    ? products
        ...
        ?.map((i) => ({ label: `${i.productName} > ${i.name}`, ...i }))
    : [];
};
export const normalizedInventories = (products) => {
  return products.length
    ? products?.map((a) => {
        if (a?.variant) {
          return { ...a?.variant, product: a.product };
        } else {
          return { ...a?.product, product: a.product };
        }
      })
    : [];
};
...
export const normalizedSubCategories = (items) => {
  return items.length
    ? items?.map((i) => ({ label: `${i.category.name} / ${i.name}`, ...i }))
    : [];
};
...
export const stateAndLgas = [
  {
    code: "FCT",
    name: "FCT",
    lgas: ["Municipal", "Kwali", "Kuje", "Gwagwalada", "Bwari", "Abaji"],
  },
  {
    code: "AB",
    name: "Abia",
